...
constexpr uint64_t max_cache_size = ((1 << 20) * 40ul);

struct KvCache{
    std::map<uint64_t,KVBlock*> cache;
    KvCache():cache(){}
    ~KvCache(){
        for(auto e:cache){
            free(e.second);
    ...
    void update_cache(uint64_t offset,Slice* key,Slice* value);
    KVBlock* get_cache(uint64_t offset);
};
