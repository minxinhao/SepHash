...
{
// 写到search_wcache才发现，在现有结构下增加filter cache实在太愚蠢了： 
// 为了检测cache是否失效，需要先读一半filter，有效再读另一半。
// 所以改为增加kv cache算了
...
constexpr uint64_t filter_cache_size = ((1 << 20) * 40ul);
...
struct FilterID{
    uint64_t level_ptr; // 因为level在动态变化，所以使用level_ptr来作为标识；可以认为level_ptr是递增的
    uint64_t buc_id;
    bool operator<(const FilterID& rhs) const{
        if(level_ptr < rhs.level_ptr) 
            return true;
        if(level_ptr == rhs.level_ptr && buc_id < rhs.buc_id) 
            return true;
        return false;
    }
};

/// @brief 建立一个从底向上的cache，
// 写到search_wcache才发现，在现有结构下增加filter cache实在太愚蠢了： 
// 为了检测cache是否失效，需要先读一半filter，有效再读另一半。
// 所以改为增加kv cache算了
struct FilterCache
{ 
    std::map<FilterID,FilterEntry> cache;

    bool is_in_cache(uint64_t level_ptr, uint64_t buc_id);
    void update_filter(uint64_t level_ptr, uint64_t buc_id,uint64_t* filter_data);
    uint64_t* get_filter(uint64_t level_ptr, uint64_t buc_id);
}__attribute__((aligned(1)));
...
private:
    task<bool> search_wcache(Slice *key, Slice *value);
